...
Use this reference after the approved source ledger and story contract exist, before drafting or revising any script. Treat it as a positive recipe for spoken writing, not a license to add material.

## Protect the writer's reality

Keep the writer's real experience intact. Use only approved source material and authorized evidence.
...
Start by deleting, not decorating. Cut throat-clearing, duplicate explanation, excess summary, softened restatements, and any explanation already demonstrated by the story. Keep the strongest source phrasing and the movement it earns.

## Performability pass

Read every line aloud. Split or cut every line that causes a stumble, restart, or breath failure.

- Format one breath per line. Line breaks are pauses, and em dashes are beats.
- Use contractions in English. In Simplified Chinese, use natural spoken compression without forcing English grammar onto it.
- Keep useful fragments when they sound true in the speaker's mouth.
- Never use parentheses, semicolons, or performative asides.

## Preserve voice and useful repetition

Preserve fragments and the source's useful edge rather than smoothing them into essay prose. Repeat a key phrase two or three times across the script when it improves listener recall. Do not repeat an explanation after the listener has already received it.

## Build the clip line

Place one clip line around the two-thirds point. Prefer a line the writer already supplied but buried under explanation. If it must be built, build it only from approved source material.
